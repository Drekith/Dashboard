...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _build_settings_tab(self) -> QtWidgets.QWidget:
        ...
        self.bcm_can_channel_input = QtWidgets.QLineEdit(
            self.current_config.bcm_can_channel or self.current_config.can_channel
        )
        port_form.addRow("BCM CAN channel", self.bcm_can_channel_input)

        self.bcm_can_bitrate_input = QtWidgets.QLineEdit(
            str(self.current_config.bcm_can_bitrate)
        )
        port_form.addRow("BCM CAN bitrate", self.bcm_can_bitrate_input)
    ...
    def _apply_port_config(self) -> None:
        ...
        config = ProviderConfig(
            ...
            bcm_can_channel=self.bcm_can_channel_input.text().strip(),
            bcm_can_bitrate=self._safe_int(
                self.bcm_can_bitrate_input.text(), self.current_config.bcm_can_bitrate
            ),
        ...
        try:
            ...
            self.hardware_status_label.setText(
                "Status: Applied ports (CAN {} / BCM {} / K-Line {})".format(
                    config.can_channel or "sim",
                    config.bcm_can_channel or config.can_channel or "sim",
                    config.kline_port or "sim",
                )
            )
            if hasattr(self, "port_summary"):
                self.port_summary.setText(
                    "Current: "
                    f"CAN {config.can_channel or 'sim'} @ {config.can_bitrate} | "
                    f"BCM {config.bcm_can_channel or config.can_channel or 'sim'} @ {config.bcm_can_bitrate} | "
