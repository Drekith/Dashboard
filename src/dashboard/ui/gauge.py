...
class GaugeWidget(QtWidgets.QWidget):
    ...
    def __init__(self, title: str, unit: str, maximum: float, style: str = "oem") -> None:
        ...
        self._base_maximum = maximum or 1.0
        self._minimum_scale = max(10.0, self._base_maximum * 0.4)
        self._value = 0.0
        self._display_value = 0.0
        self._style: dict[str, str] = {}
        self._style_name = ""

        self._recent_values: deque[float] = deque(maxlen=32)
        self._recent_values.append(0.0)
    ...
    def setValue(self, value: float) -> None:  # noqa: N802
        ...
        self._recent_values.append(incoming)
        peak = max(self._recent_values)
        target_for_scale = max(incoming, peak) * 1.08
        target_for_scale = max(target_for_scale, self._minimum_scale)

        if target_for_scale > self.maximum * 1.02 or target_for_scale < self.maximum * 0.7:
            self._recalculate_scale(target_for_scale)
