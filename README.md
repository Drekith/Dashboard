...
- Open the **Settings** tab (or tap the **Ports** quick button on the Dashboard header) to edit CAN, BCM-CAN, and K-Line channels/bitrates while the app is running. Click **Apply port settings** to rebuild the providers with the new COM assignments without restarting.
- Use **Auto-detect K-Line** to prefill the first detected ELM/OBD serial device; detected ports are also listed in the dropdown for quick selection. The current CAN/BCM/K-Line settings are shown above the form for quick confirmation.
