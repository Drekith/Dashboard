...
1. Plug in the adapters and note their Windows COM names (e.g., `COM3` for CAN/BCM, `COM4` for ELM327). Export them as environment variables before launching. If `KLINE_PORT` is not set, the app will try to auto-detect the first ELM/OBD USB serial device:

```bash
set CAN_CHANNEL=COM3
set BCM_CAN_CHANNEL=COM3
set KLINE_PORT=COM4
# Optional: override bitrates if your BCM line is low-speed
set BCM_CAN_BITRATE=33333
set CAN_BITRATE=500000
```
...
- Open the **Settings** tab (or tap the **Ports** quick button on the Dashboard header) to launch pop-up dialogs for CAN, BCM CAN, and K-Line. Each dialog lets you edit channel/bitrate (CAN/BCM) or COM/baud (K-Line) and applies the changes live when you click **OK**.
- Use the **Auto-detect** button inside the K-Line dialog to prefill the first detected ELM/OBD serial device; detected ports are also listed in the dropdown for quick selection. The current CAN/BCM/K-Line settings and simulator state are shown above the buttons for quick confirmation.
- A **Simulator fallback** checkbox in Settings lets you toggle the built-in simulator on or off; when disabled and no hardware is active, the UI will surface a warning instead of injecting simulated frames.

## Customizing decoding

- **CAN (driveline):** Update `_decode_message` in `src/dashboard/providers/can_provider.py` if you attach the Waveshare dongle to a high-speed CAN line for cluster data.
...
- Typical driveline CAN bitrate is 500 kbps; adjust `bitrate` in `CanProvider` if your van differs.
